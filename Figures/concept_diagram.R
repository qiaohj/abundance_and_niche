...
library(dplyr)
library(rayshader)
source("commonFuns/background.R")
...
for (t in c("uniform", "gradientSimple", "two_centers_3", "random")){
  ...
  r_matrix<-as.matrix(result2$r_reproduction_rate)
  png(filename=sprintf("../Figures/concept_diagram/%s.png", t), width=3000, height=3000)
  r_matrix %>%
    sphere_shade(texture = "imhof3") %>% 
    add_shadow(ray_shade(r_matrix)) %>% 
    add_shadow(ambient_shade(r_matrix)) %>%
...
png(filename=sprintf("../Figures/concept_diagram/%s.png", "real_world"), width=3000, height=3000)
r_matrix %>%
  sphere_shade(texture = "imhof3") %>% 
  add_shadow(ray_shade(r_matrix)) %>% 
  add_shadow(ambient_shade(r_matrix)) %>%
...
plot(alt_crop)
r_matrix<-as.matrix(alt_crop)
png(filename=sprintf("../Figures/concept_diagram/%s.png", "quantion_mark"), width=3000, height=3000)
r_matrix %>%
  sphere_shade(texture = "imhof3") %>% 
  add_shadow(ray_shade(r_matrix)) %>% 
  add_shadow(ambient_shade(r_matrix)) %>%
